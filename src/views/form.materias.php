    ...
    <div class="m-2 py-4 bg-gray-100 shadow-md rounded-lg w-2/3 mx-auto">
        ...
        <form id="materias_form" class="m-2 grid grid-cols-1 md:grid-cols-2 gap-4" action="materias" method="POST">
            <div id="materias_camps">
                <?php
                if (isset($_SESSION['contador_materia']) && $_SESSION['contador_materia'] > 0) {
                    for ($i = 1; $i <= $_SESSION['contador_materia']; $i++) {
                        if (isset($_SESSION['materia' . $i])) {
                            $materia = $_SESSION['materia' . $i];
                            ?>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-700" for="nombre_materia">Nombre</label>
                                <input class="block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="nombre_materia[]" placeholder="Nombre" value="<?= $materia['nombre']; ?>" required>
                            </div>
                            <br>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-700" for="curso_materia">Curso</label>
                                <input class="block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="curso_materia[]" placeholder="Curso" value="<?= $materia['curso']; ?>" required>
                            </div>
                            <br>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-700" for="duracion_materia">Duración</label>
                                <input class="block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="number" name="duracion_materia[]" placeholder="Duración" value="<?= $materia['duracion']; ?>" required>
                            </div>
                            <hr class="mt-10">
                            <?php
                        }
